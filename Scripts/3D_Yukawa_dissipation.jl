...
if !isfile("Data/Loss/Yukawa_Local_Loss_U$(U0)_λ$(λ).jld2")
    pos_particle, speed_particle = solve_particle()
    save_object(
        "Data/Loss/Yukawa_Local_Loss_U$(U0)_λ$(λ).jld2",
        (pos_particle, speed_particle),
    )
end

if !isfile("Data/Loss/Yukawa_Full_Loss_U$(U0)_λ$(λ).jld2")
    pos, speed = solve_full()
    save_object("Data/Loss/Yukawa_Full_Loss_U$(U0)_λ$(λ).jld2", (pos, speed))
end

## PLOTTING

pos_local, speed_local = load_object("Data/Loss/Yukawa_Local_Loss_U$(U0)_λ$(λ).jld2")
pos_full, speed_full = load_object("Data/Loss/Yukawa_Full_Loss_U$(U0)_λ$(λ).jld2")
...
# xlims!(ax_pos, (0,30))
# xlims!(ax_speed, (0,30))
hlines!(ax_pos, [10.5], color = CF_black)
hlines!(ax_speed, [0], color = CF_black)
axislegend(ax_speed, framevisible = false, position = :rb)
# polys =
...
fig
save("Yukawa_Trapped_Dissipation.pdf", fig)
dd = load_object("Data/Loss/Yukawa_Local_Loss_U500_λ0.5.jld2")
lines([x[3] for x in dd[1]])
