...
Threads.@threads for p in params
    ...
    λ = p[2]
    U0 = p[3]
    if !isfile("Data/Reflection/Full_Reflection_U$(U0)_λ$(λ)_init_speed$(init_speed).jld2")
        @inline function U(r)
        ...
        save_object(
            "Data/Reflection/Full_Reflection_U$(U0)_λ$(λ)_init_speed$(init_speed).jld2",
...
titles = [
    L"\lambda = 3/8\mathrm{\AA},\,\dot{R} = 5\mathrm{\AA/ps}",
    L"\lambda = 3/4\mathrm{\AA},\,\dot{R} = 5\mathrm{\AA/ps}",
    L"\lambda = 3/8\mathrm{\AA},\,\dot{R} = 2\mathrm{\AA/ps}",
    L"\lambda = 3/4\mathrm{\AA},\,\dot{R} = 2\mathrm{\AA/ps}",
]
